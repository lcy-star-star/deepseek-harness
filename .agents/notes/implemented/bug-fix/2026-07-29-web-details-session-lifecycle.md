...
`AppFrame` reads the current Session id from the authoritative Session projection and records any connected selected Session as the details owner. A connected blank Session can own an explicitly opened auxiliary details surface under the [explicit-details refinement](2026-08-29-explicit-details-on-blank-sessions.md); states with no current Session neither trigger closure nor replace the last Session owner, and their rendered details track derives as zero without changing the stored preference. The first Session preserves the layout store's initial preference, whose [archived visibility-default decision](../../archived/bug-fix/2026-07-30-web-details-default-closed.md) chose closed; returning to the same Session restores its current width, and selecting a different Session closes the root-scoped details preference through the layout store before paint. The per-Session chat selection remains owned by the session-scoped store described by the [slot system standard](../architecture/2026-07-22-slot-type-chain-implementation.md).
...
Details starts closed, including when the first Session materializes. An explicit open action uses the contract default width for either a blank or non-blank connected Session. Switching to a different Session forgets a dragged details width because close writes zero and reopen uses that default. States with no current Session derive a zero rendered track while leaving the preferred geometry unchanged; returning to the same Session through one of those states restores its width. Reload forgets sidebar geometry and restores details closed. The layout behavior test covers initial defaults, explicit opening on a blank Session, first materialization, direct and hero-mediated Session switches, same-Session return, and the absence of layout storage; the keyless browser e2e drives the same owner transitions through the shipped composition while checking the full grid track and browser errors.
