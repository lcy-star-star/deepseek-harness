...
`ctx.settingsNavigation` exposes the shell's existing viewing state to other client plugins. `open(sectionId?)` opens the same dialog at a registered `settings.section` id, `close()` closes it, and its observable snapshot drives the trigger and panel. It creates no alternate settings page and writes no configuration value.
