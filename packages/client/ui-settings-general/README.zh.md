...
`ctx.settingsNavigation` 把此外壳既有的查看状态暴露给其他客户端插件。`open(sectionId?)` 会在某个已注册的 `settings.section` id 处打开同一个对话框，`close()` 会将其关闭，触发行和面板由其可观察快照驱动。它不会建立另一张设置页面，也不会写入任何配置值。
