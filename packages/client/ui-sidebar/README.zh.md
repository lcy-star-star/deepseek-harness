...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.before.workspaces`、`sidebar.workspaces`、`sidebar.after.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。两个可叠加的列表 seat 会在同一滚动区域内紧邻 Workspace 浏览器的前后渲染，并接收其 `wide` 与 `expandSidebar` owner share；它们不会替换或重新解释 `sidebar.workspaces`。
