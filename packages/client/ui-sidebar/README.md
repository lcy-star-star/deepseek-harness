...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.before.workspaces`, `sidebar.workspaces`, `sidebar.after.workspaces`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store. The two additive list seats render immediately before and after the Workspace browser inside the same scrolling region and receive its `wide` and `expandSidebar` owner share; they do not replace or reinterpret `sidebar.workspaces`.
