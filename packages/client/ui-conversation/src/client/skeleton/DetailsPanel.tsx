...
export function DetailsPanel({
  ...
  closeDetails, showAuxiliary, hasAuxiliary, t,
}: DetailsPanelProps) {
  ...
  const auxiliary = hasAuxiliary
    ? renderSlot('conversation.details.auxiliary', { closeDetails })
    : emptyDetails

  return (
    <div className={css.root}>
      <div hidden={selection !== null} className={css.auxiliary}>
        {auxiliary}
      </div>
      {selection !== null && (
        <>
          <div className={css.header}>
            <div className={css.headerLeading}>
              {hasAuxiliary && (
                <button type="button" className={css.back} aria-label={t('details.back')} onClick={() => { showAuxiliary() }}>
                  <svg viewBox="0 0 16 16" width="14" height="14" aria-hidden>
                    <path d="M10.5 3.5L6 8l4.5 4.5" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </button>
              )}
              <div className={css.title}>{material?.name ?? selection.toolName ?? t('details.title')}</div>
            </div>
            <button
              type="button" className={css.close} aria-label={t('details.close')}
              onClick={() => { closeDetails() }}
          ...
          <div className={css.body}>
            {callId === undefined || material === null
